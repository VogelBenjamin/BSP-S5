...
inlet_dof = locate_dofs_topological(velocity_fem_space, 1, boundaries.find(2))
...
v_inlet = np.zeros(mesh.geometry.dim).astype(PETSc.ScalarType)
v_inlet[1] = 1
...
inlet_bc = dirichletbc(v_inlet, inlet_dof, velocity_fem_space)
outlet_bc = dirichletbc(p_outlet, outlet_dof, pressure_fem_space)

u_bc = [walls_bc,inlet_bc]
p_bc = [outlet_bc]
